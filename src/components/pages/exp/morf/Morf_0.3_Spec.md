...
5.  **行为与身份的代数统一**：通过 Mixin 算符 `+:`，将传统语言中的枚举（Enum/Sum Type）与接口实现（Impl/Trait）统一为同一数学操作——**数据与元数据的叠加**。枚举即“数据混入身份标签”，Impl 即“数据混入行为集合”。这在底层彻底消融了 OOP（行为扩展）与 FP（代数数据类型）的边界。
...
通过 `Nominal.CreateMorph` 创建的具名构体（NamedMorph）不仅仅是类型定义，它自动获得**构造器**能力。

#### 构造调用语法
具名构体 `T` 可以像函数一样被调用：`T { ...args }`。这提供了一种基于原型的对象构造方式。
...
| 12 | 类型算符 | `&` (交集), `\|` (并集), `+:` (混入) | 左(Mixin右) | 类型逻辑层运算 |
...
#### 3.2.2 混入与类型元数据 (Type Meta)

混入运算符 `+:` 不仅用于构造逻辑并集，更是 Morf 中将 **行为（Behavior）** 与 **数据（Data）** 绑定的核心机制。

* **语法**: `Type +: Meta`
* **结合性**: 右结合。
* **语义**:
    1.  **逻辑并集**: 在子类型判定中，`A +: B` 等价于 `A | B`。
    2.  **结构覆盖**: 在值层作为构体使用时，结构特征完全由 `B` 决定。
    3.  **元数据绑定 (Type Meta)**: 
        *   `B` 被视为 `A` 的 **类型元数据 (Type Meta)** 或 **伴生实现**。
        *   当对类型为 `A +: B` 的值进行属性访问时，若值本身缺失该属性，系统将自动在 `B` 中查找。这构成了 Morf 统一方法分派的基础（见 10.3）。
        *   **注**：Impl 机制在本质上就是隐式的、延迟绑定的 Mixin 操作（见 10.3.1）。

**语法糖：标签类型**

`'Tag Value` 是混入的一种特例，用于将标签元数据绑定到值上。
...
* **示例**:
  ...
  // Color 逻辑上等价于 1 | 2 | 3
  let c: Color = 1 // 合法: 1 <: (1 | 2 | 3)
  
  // Color 结构上拥有 Red, Green, Blue 属性
  Color.Red // -> 1
  ```
...
* **构造行为**: 在使用具名构体构造器（见 2.3.10）时，若未提供该属性的值，则其值默认为 `None`。
...
### 5.3 区间类型系统（Interval Type System）

为了在类型系统中表达数值约束，Morf 引入了 **Interval** 体系。

**设计边界**：
- **语言核心**：提供区间类型构造器（`Lt{N}`, `Gt{N}`, `IntervalXX{Min, Max}`）及其子类型关系。
- **标准库/第三方库**：提供区间值的构造函数与运算实现（如区间算术）。

区间类型是类型层的工具，用于约束与推导；具体的数值运算由 Impl 系统实现。
...
#### 5.3.4 类型逻辑归约（Type-Logic Reduction）

在类型逻辑层，区间类型通过类型算符（`&`, `|`, `~`）进行归约。以下归约规则由编译器保证，用于类型检查与类型推导：

**交集归约**（`&` 算符）：
*   `Lt{5} & Lt{3}` ≡ `Lt{3}` — 取更严格的上界
*   `Gt{1} & Gt{3}` ≡ `Gt{3}` — 取更严格的下界
*   `Gt{1} & Lt{3}` ≡ `IntervalOO{1, 3}` — 构造有界区间
*   `Lt{1} & Gt{3}` ≡ `Never` — 空集（矛盾约束）

**并集归约**（`|` 算符）：
*   `Lt{3} | Lt{5}` ≡ `Lt{5}` — 取更宽松的上界
*   归约为 `DiSet` 当无法合并时（见 2.3.7）

**补集**（`~` 算符）：
*   `~Lt{5}` ≡ `Gt{5} | {5}` — 语义模型层解释
*   实现可采用等价的逻辑表示

**重要**：上述归约仅在类型逻辑层生效。值层的区间运算（如两个区间值相加）应由标准库或第三方库通过 Impl 系统提供，不属于语言核心职责。

#### 5.3.5 值层运算的设计空间（非规范性）

区间值的算术运算（加、减、乘、除等）不由语言规范定义。不同的库可以根据领域需求采取不同策略：

**示例：区间除法遇到跨零分母**
```morf
// 假设有库实现的区间值
let a = SomeLib.Interval.Of{ 2, 4 }
let b = SomeLib.Interval.Of{ -2, 4 }  // 跨越 0

// 策略 1（精确但复杂）：返回不连续的多区间
a / b  // -> DiSet{ Interval{-Inf, -1}, Interval{0.5, Inf} }
...
// 不同库可以做出不同选择，语言仅提供类型工具支持这些选择
```

**类型系统的角色**：
- 类型层的 `Lt{N}`, `Gt{N}` 等用于**约束推导**（如 `x: Gt{0}` 可优化除零检查）
- 值层的区间运算结果类型可以是 `Interval`、`DiSet{Interval...}`、`Number` 等，由库决定
- 用户可以通过类型标注选择所需的精度与复杂度权衡
...
**Impl 与名义**：候选集限定于具名构体/DiSet{具名, None}（10.3.2），且脱糖规则固定（10.3.3）。在无动态 `handle` 的上下文、且接收者类型可静态确定时，可将 `.` 调用完全解析为静态调用，便于内联与去虚拟化。
...
**严重警告**：不得依赖 `A[NominalKey]` 来判断 “x 是不是 A 的实例”（即试图把类型对象 `A` 当成实例的原型来读取 `NominalKey`）。

在 Morf 的规范里：

*   **判断实例**：必须使用 `x <: A`。
    *   这走的是**类型逻辑侧**，是类型安全的。
*   **判断相等**：使用 `x[NominalKey] == A[NominalKey]`。
    *   这走的是**结构值侧**，此时 `A` 被视为一个具体的 Impl 值（构体）。
...
#### 10.3.1 统一访问规则：Impl 即 Mixin

Morf 的 Impl 机制是一种**隐式的、延迟绑定的 Mixin 操作**。

表达式 `E.Key` 的解析逻辑遵循以下数学模型：

$$ \text{EffectiveObject} = \text{ImplSet} \ +: \ \text{Data} $$

其中 `+:` 是 **Mixin 算符**（见 3.2.2），遵循 **“右侧优先/后来者覆盖”** 的语义。
*   **Data** (`E`) 位于右侧，拥有最高优先级。
*   **ImplSet** 位于左侧，作为被混入的底座或默认实现。

因此，访问 `E.Key` 等价于在上述 `EffectiveObject` 上访问 `Key`：
1.  **Data 优先**：如果 `Data` 中存在 `Key`，由于 `Data` 在右侧，它会直接覆盖左侧 `ImplSet` 中的同名属性。
2.  **Impl 兜底**：如果 `Data` 中不存在，则“透视”到左侧，访问 `ImplSet` 中的属性（通常是方法）。

#### 10.3.2 Impl 候选集合与名义索引

如果 Impl 只是 Mixin，为什么我们需要限制查找范围？
**因为编译器需要知道“去哪里找左操作数（ImplSet）”**。

对于纯结构化数据（如 `{ x: 1 }`），编译器无法知道应该混入哪个 Impl。因此，**名义属性（NominalKey）** 充当了查找左操作数的**索引**。

Impl 查找 **仅允许**在以下候选集合中进行，因为只有这些集合可以通过名义索引被唯一确定：

1. **`impl for 具名构体`**：目标类型为某个具名构体的 impl；
2. **`impl for DiSet{ 具名构体, None }`**：目标类型为 `DiSet{N, None}`（即 `N | None`）这一类形态的 impl；
3. **`impl` 构体自身**：即候选必须是带有 `ImplNominal` 标记的构体。

超出以上范围的 `impl`（例如纯结构目标类型 `for { value: Uni }`）由于缺乏索引，**不得**被 `.` 直接自动命中；必须使用 `<impl_id>.` 显式指定（见 10.4），这实际上是手动指定了左操作数。
...
- 候选范围内找不到适用实现；
- 用户希望使用但不在允许范围内的实现（例如纯结构目标类型的 impl）；
- 用户希望绕过默认选择规则，强制指定某个实现体。

这等价于显式构造了一个 Mixin 操作。

**数学等价性**：
$$ \text{obj}\texttt{<Trait>}.\text{method}\{\} \equiv (\text{Trait} \ +: \ \text{obj}).\text{method}\{\} $$
...
#### 10.5.1 Impl 继承即 Mixin 组合

`impl Child extends Parent { ... }` 声明 `Child` 继承 `Parent` 的方法集合，并允许对同名条目进行覆盖。

在 Mixin 模型下，这等价于：
$$ \text{Child} = \text{ChildBody} \ +: \ \text{Parent} $$

这意味着 `Child` 是一个复合的 ImplSet，它首先包含 `Parent` 的所有定义，然后由 `ChildBody`（花括号内的定义）进行覆盖或扩展。
...
这直接遵循 Mixin 的 **“右侧/后来者覆盖”** 规则：
*   在 $\text{ChildBody} \ +: \ \text{Parent}$ 中，`ChildBody` 位于右侧（后来者）。
*   因此 `ChildBody` 中的 `Invert` 覆盖了 `Parent` 中的同名方法。

由于 Morf 中“数据就是类型”，因此 impl 本身也处于类型关系与构体结构之中；**签名一致**的覆盖是合法的语言行为。

#### 10.5.3 `super` 语义

在 `Child` 的方法体内部，`super.Method{ ... }` 表示调用被覆盖的父级实现。

在 Mixin 模型中，这等价于访问 Mixin 链中 **“当前层左侧”** 的那一部分。
...
这等价于：
$$ \text{VirtualView} = \text{Impl}_{\text{later}} \ +: \ \text{Impl}_{\text{earlier}} \ +: \ \text{Data} $$
...
* **impl NominalImpl for Nominal**:
  ...
  * **static CreateMorph{ ... }**: 创建具名构体。该构体自动获得构造器能力（见 2.3.10）。
  * **static CreateMorphOf{ set: NominalSet, ns: Uni }**: 创建具名构体。该构体自动获得构造器能力（见 2.3.10）。
