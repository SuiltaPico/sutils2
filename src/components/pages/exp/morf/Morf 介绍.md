...
## 依赖即空气
传统编程需要复杂的依赖注入框架（Spring, NestJS）来解耦业务与实现。Morf 将依赖管理内化为语言底层的 **Effect (效应)** 机制。
- **业务编写者**：只管表达意图（如 `DB.Save`），无需关心实现细节（MySQL/Mock），也无需编写繁琐的构造函数注入。
- **系统实现者**：通过 Effect Handler 在运行时动态注入具体实现。
- **结果**：业务逻辑极其纯净，因为依赖像空气一样无处不在，却又不可见。这是让非程序员编写可运行业务逻辑的关键。
